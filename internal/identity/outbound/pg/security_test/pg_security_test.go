...
// Security-focused tests for the identity pg package.
//
// Each test is either:
...
//
// Run with: go test -race -failfast ./internal/identity/outbound/pg/security_test/...

// NOTE: This was originally a white-box test file (package pg) that inspected
// unexported symbols and embedded SQL strings via source file analysis. Since
// all checks use os.ReadFile / go/parser on source files (not unexported Go
// symbols), they work identically from an external test package.
...
// pgDir returns the absolute path to the parent pg package directory.
func pgDir() string {
	_, thisFile, _, _ := runtime.Caller(0)
	return filepath.Dir(filepath.Dir(thisFile))
}

func readPgFile(t *testing.T, name string) []byte {
	t.Helper()
...
func TestSecurity_VULN05_PgcryptoEitherUsedOrRemoved_Green(t *testing.T) {
	migSrc := readPgFile(t, "migrations/001_identity.sql")
	pgGoSrc := readPgFile(t, "pg.go")
...
func TestSecurity_VULN07_SensitiveFieldsUseEncryptedType_Green(t *testing.T) {
	...
	src, err := os.ReadFile(filepath.Join(pgDir(), "../../domain/types.go"))
	require.NoError(t, err)
