...
// dropKanbanTables removes all kanban tables so tests start from a clean slate.
func dropKanbanTables(t *testing.T, pool *pgxpool.Pool) {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func TestSecurity_PoolMaxConns_RED(t *testing.T) {
	...
	repos, err := kanbanpg.NewRepositories(pool)
