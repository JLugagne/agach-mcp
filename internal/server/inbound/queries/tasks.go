...
func (h *TaskQueriesHandler) GetBoard(w http.ResponseWriter, r *http.Request) {
	...
	boardColumns := make([]pkgserver.ColumnWithTasksResponse, len(columns))
	for i, col := range columns {
		...
		boardColumns[i] = pkgserver.ColumnWithTasksResponse{
			ColumnResponse: converters.ToPublicColumn(col),
			Tasks:          allTasks,
		}
	}

	h.controller.SendSuccess(w, r, pkgserver.BoardResponse{
...
func (h *TaskQueriesHandler) ListTasksByAgent(w http.ResponseWriter, r *http.Request) {
	...
	h.controller.SendSuccess(w, r, pkgserver.TasksByAgentResponse{
		AgentSlug: agentSlug,
